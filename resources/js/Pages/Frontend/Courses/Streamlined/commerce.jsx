...
// ------------------------------------------------------------------
// Icon resolver — DB stores icon as a string (e.g. "Calculator")
// ------------------------------------------------------------------
const ICON_MAP = {
  Layers3,
  BookOpen,
...
function resolveIcon(name, fallback = Receipt) {
  return ICON_MAP[name] || fallback;
}

// ------------------------------------------------------------------
// Static "Quick facts" per degree option — not stored in DB, so kept
...
const STATIC_DEGREE_FACTS_BY_TITLE = [
  { match: /b\.?com.*(general|honours)/i, facts: STATIC_DEGREE_FACTS["bcom-general-honours"] },
  { match: /bba/i, facts: STATIC_DEGREE_FACTS["bba"] },
  { match: /b\.?com.*(accounting|finance)/i, facts: STATIC_DEGREE_FACTS["bcom-accounting-finance"] },
  { match: /economics/i, facts: STATIC_DEGREE_FACTS["economics"] },
];
...
// Page
// ------------------------------------------------------------------
export default function CommerceDegree({ courseContent: courseContentProp }) {
  const { props } = usePage();
  ...
  return (
    <>
      <FrontendLayout>
        <HeroInner
          title={intro_heading || "Commerce Graduation Courses (B.Com & Allied)"}
          breadcrumb="B.Com & Allied"
        />
        <CoursesTabsBar tabs={TABS} activeId="bcom" />
          ...
          <div className="container">
            <div className="row g-4 align-items-start">
              <div className="col-12 col-lg-7">
                <h2 className="sectionHeading mb-3 d-flex align-items-center gap-2">
                  <BookOpen size={18} className="text-primary" />
                  <span>{intro_heading || "About Commerce Graduation Courses"}</span>
                </h2>
        ...
        {/* 4) After degree: sensible next steps */}
