...
function resolveIcon(name, fallback = Calculator) {
...
const STATIC_DEGREE_FACTS_BY_TITLE = [
  { match: /allied/i, facts: STATIC_DEGREE_FACTS["allied-commerce-pg"] },
  { match: /specialised|specialized|specialisation/i, facts: STATIC_DEGREE_FACTS["mcom-specialised-streams"] },
  { match: /m\.?com.*(general|accounting|finance)/i, facts: STATIC_DEGREE_FACTS["mcom-general-accounting-finance"] },
];
...
// Page
// ------------------------------------------------------------------
export default function CommercePGDegree({ courseContent: courseContentProp }) {
  const { props } = usePage();
  ...
  return (
    <>
      <FrontendLayout>
        <HeroInner
          title={intro_heading || "Commerce PG Courses (M.Com & Allied)"}
          breadcrumb="M.Com & Allied"
        />
        <CoursesTabsBar tabs={TABS} activeId="mcom" />
          ...
          <div className="container">
            <div className="row g-4 align-items-start">
              <div className="col-12 col-lg-7">
                <h2 className="sectionHeading mb-3 d-flex align-items-center gap-2">
                  <BookOpen size={18} className="text-primary" />
                  <span>{intro_heading || "About Commerce Post Graduation"}</span>
                </h2>
