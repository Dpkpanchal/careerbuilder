...
// collegeContent = the existing record being edited, passed in as a prop
// from CollageContentController@edit
const CollegeEdit = ({ tabsData, menu_id, collegeContent }) => {

    const { data, setData, put, processing, errors } = useForm({

        menu_id: collegeContent?.menu_id ?? menu_id ?? "",
        tab_id: collegeContent?.tab_id ?? "",
        section_id: collegeContent?.section_id ?? "",
        link_id: collegeContent?.link_id ?? "",

        name: collegeContent?.name ?? "",
        code: collegeContent?.code ?? "",
        state: collegeContent?.state ?? "",
        city: collegeContent?.city ?? "",
        website: collegeContent?.website ?? "",
        address: collegeContent?.address ?? "",
        contact: collegeContent?.contact ?? "",
        note: collegeContent?.note ?? "",
        is_active: collegeContent?.is_active ?? true,
    ...
    const submit = (e) => {
        ...
        put(`/admin/college-content/${collegeContent.id}`);

    };

    return (

        <AdminLayout header="Edit College Content">

            <Head title="Edit College Content" />
