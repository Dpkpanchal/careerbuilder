...
export default function Index({ examContents, filters = {} }) {

    const [linkSearch, setLinkSearch] = useState(filters.link || "");
    const [selectedStatus, setSelectedStatus] = useState(filters.status || "");
    const [expandedSections, setExpandedSections] = useState(() => {
        // By default, expand all sections
        const initial = {};
        if (examContents?.data) {
            examContents.data.forEach((section, index) => {
                initial[index] = true;
            });
        }
        return initial;
    ...
    const {
        ...
        to,
        total,
    } = examContents || { data: [], links: [], from: null, to: null, total: 0 };
    ...
    const applyFilters = (overrides = {}) => {
        const params = {
            link: overrides.link !== undefined ? overrides.link : linkSearch,
        ...
        router.get("/admin/exam-content", params, {
            preserveScroll: true,
            preserveState: true,
        });
    };
    ...
    const clearFilters = () => {
        setLinkSearch("");
        setSelectedStatus("");
        router.get("/admin/exam-content", {}, {
            preserveScroll: true,
            preserveState: true,
        });
    };

    const hasActiveFilters = linkSearch || selectedStatus;
            ...
            <div className="container-fluid">
                <div className="card exam-card">
                    <div className="card-header">
                        <div className="row align-items-center">
                            <div className="col-md-6">
                                <h3 className="card-title mb-0">Exam Content</h3>
                            </div>
                            <div className="col-md-6 text-right">
                                <Link href="/admin/exam-content/create" className="btn btn-primary">
                    ...
                    <div className="filter-bar">
                        <input
                            type="text"
                            placeholder="Search by exam, URL, tag..."
                            value={linkSearch}
                            onChange={(e) => setLinkSearch(e.target.value)}
                            onKeyDown={handleLinkKeyDown}
                            onBlur={() => applyFilters({ link: linkSearch })}
                        />
                    ...
                    <div className="card-body">
                        {sections.length > 0 ? (
                            sections.map((section, index) => {
                                ...
                                const activeCount = section.items?.filter(item => item.is_active).length || 0;

                                return (
                                    <div key={section.section_id || index} className="section-card">
                                        {/* Section Header */}
                                        <div 
                                        ...
                                        >
                                            <div className="section-info">
                                                <span className="section-title">
                                                    {section.section_label || `Section #${section.section_id}`}
                                                </span>
                                            ...
                                            <div className="d-flex align-items-center" style={{ gap: '0.5rem' }}>
                                                {/* <Link
                                                    href={`/admin/exam-content/section/${section.section_id}/edit`}
                                                    className="btn btn-sm btn-primary edit-section-btn"
                                                            ...
                                                            section.items.map((item, itemIndex) => (
                                                                <tr key={item.id}>
                                                                    ...
                                                                    <td className="font-weight-600">
                                                                        {item.exam || item.name || item.title || item.route || 
                    ...
                    {sections.length > 0 && (
                        <div className="pagination-wrap">
                            <div className="pagination-info">
                                Showing {from} to {to} of {total} sections
                            </div>
                            <nav>
