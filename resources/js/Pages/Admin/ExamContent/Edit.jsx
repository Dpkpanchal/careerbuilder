...
// examContent = the existing record being edited, passed in as a prop
// from ExamContentController@edit
const ExamEdit = ({ menus, examContent }) => {
    ...
    const { data, setData, put, transform, processing, errors } = useForm({
        ...
        fullForm: examContent?.full_form ?? "",
        ...
        eligibility:
            examContent?.eligibility && examContent.eligibility.length > 0
                ? examContent.eligibility
                : [""],
        apply: examContent?.apply ?? "",
        activity:
            examContent?.activity && examContent.activity.length > 0
                ? examContent.activity
                : [""],
        ...
        linkLabel: examContent?.link_label ?? "",

        // Notes
        note: examContent?.note ?? "",
        statusNote: examContent?.status_note ?? "",

        // Others
        wbFocus: examContent?.wb_focus ?? false,
        is_active: examContent?.is_active ?? true,

        group: examContent?.group ?? "",
        sl: examContent?.sl ?? "",

        // Multiple sources / links
        sources:
            examContent?.sources && examContent.sources.length > 0
                ? examContent.sources
                : [
                    {
                        label: "",
    ...
    const submit = (e) => {
        ...
        // Inertia's put() sends `data` as-is. Strip blank entries out of the
        // array fields before sending, and send null instead of an empty
        // array when nothing was filled in (keeps it consistent with the
        // migration's NULL handling). Keys are left as camelCase — the
        // update() controller expects the same shape store() does.
        transform((formData) => {
        ...
        put(`/admin/exam-content/${examContent.id}`);

    };

    return (

        <AdminLayout header="Edit Exam">
