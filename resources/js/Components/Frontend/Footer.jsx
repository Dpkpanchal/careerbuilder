...
export default function PortalFooterMinimal() {
  ...
  const data = useMemo(() => {
    ...
    return {
      ctas,
      columns: {
        careers: topN(uniqByHref([...stage, ...profession, ...future]), 12),
        courses: topN(uniqByHref(coursesAll), 12),
        colleges: topN(uniqByHref(collegesAll), 10),
        exams: topN(uniqByHref(examsAll), 8),
        support: topN(
          uniqByHref([
            { 
